...
CREATE OR REPLACE FUNCTION insert_command_and_event_with_row_versioning(
    ...
    p_expected_next_sequence INTEGER
)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
    ...
    v_next_now INTEGER;
BEGIN
    IF p_expected_next_sequence < 1 THEN
    ...
    IF v_next_now IS DISTINCT FROM p_expected_next_sequence THEN
        RAISE EXCEPTION 'row version conflict: expected next sequence %, actual next % (aggregate % / %)',
            p_expected_next_sequence, v_next_now, p_event.aggregate_type, p_event.aggregate_id
            USING ERRCODE = 'P0001';
    END IF;
